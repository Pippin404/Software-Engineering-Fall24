...
//        I'm not sure how to use junit to test this so I'm just using main for now
        ...
        OutputConfig outputConfig = new OutputConfig(
                "C:\\Users\\eribr\\Desktop\\Software Engineering\\Software-Engineering-Fall24\\test\\ds\\iotests\\OutputText.txt",
                OutputType.TEXT);
        ...
        InternalWriteIntegerRequest internalWriteIntegerRequest = InternalWriteIntegerRequest.builder()
                .outputPath(outputPath).outputType(outputType).build();
    ...
    // @Test
    // Can't do this one, grpc cannot build during tests
    public void parseFileWithInvalidPath() {
        DataStore dataStore = new DataStore();
        FileParseRequest fileParseRequest = FileParseRequest.builder().inputFile(new File("wombo"))
                .inputType(InputType.CSV).delimiter(Delimiter.COMMA).build();
...
//        FileParseRequest fileParseRequest = new FileParseRequest(new InputConfig(new File("test/ds/iotests/csvTest.txt"), InputType.CSV), Delimiter.COMMA);
        FileParseRequest fileParseRequest = FileParseRequest.builder()
                .inputFile(new File("test/ds/iotests/csvTest.txt")).inputType(InputType.CSV).delimiter(Delimiter.COMMA)
                .build();
