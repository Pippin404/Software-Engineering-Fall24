...
public class UScomputerEngineConstructer {
    ...
    // changed "datastore dataStore" to "DataStoreInterface dataStore", to allow a
    // non grpc version of the code to run
    // tests on. Use THIS in a new file
    // TODO: Split the grpc functionality to another file. to allow tests to avoid
    // grpc.
    ...
    // TODO: this list of results should probably be gotten through a network call,
    // maybe
    ...
    public UScomputerEngineConstructer(InternalComputeEngine computeEngine, DataStore dataStore) {
        ...
        // Keeping this dataStore so it doesn't break everything else that uses this
        // class
    ...
    public void setData() {
        ...
        ParseFileServiceRequest request = ParseFileServiceRequest.newBuilder().setInputFile(inputFile.getAbsolutePath())
                .setInputType(protobuf.datastore.CommonEnums.InputOutputType.CSV)
                .setDelimiter(protobuf.datastore.CommonEnums.ExternalDelimiter.COMMA).build();
    ...
    // TODO: right now there's only support for writing all the integers at once,
    // there should be support for writing a single integer so we can do it over a
    // stream
    public void writeComputedResultsToFile(List<Integer> results, String outputPath) {
        ...
        // TODO: repeatedly making network calls like this in a for loop might be
        // abysmal for performance but who knows
        for (int result : results) {
            WriteIntegerServiceRequest request = WriteIntegerServiceRequest.newBuilder().setComputedInteger(result)
                    .setOutputFile(outputPath)
                    // TODO: hardcoded to csv for now
                    .setOutputType(InputOutputType.CSV).build();
